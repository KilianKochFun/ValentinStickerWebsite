...
function openPunchingBag() {
  ...
  punchesLeft = pendingScore;
  document.getElementById('sqGameover').classList.remove('visible');
  document.getElementById('sqPunchSection').classList.remove('hidden');
  document.getElementById('sqPunchBtn').disabled = true;
  updatePunchRemaining();
  if (!bagInitialized) { initBag(); bagInitialized = true; }
}
...
function updatePunchRemaining() {
  const el = document.getElementById('sqPunchRemaining');
  if (punchesLeft > 0) {
    el.textContent = `Noch ${punchesLeft} Schlag${punchesLeft !== 1 ? 'e' : ''} übrig`;
    el.style.color = '#555';
  } else {
    el.textContent = '🎉 Alle Schläge verbraucht!';
...
function initBag() {
  ...
  function tryPunch(cx,cy){
    ...
    for(let i=0;i<11;i++){
      ...
      particles.push({x:cx+(Math.random()-0.5)*16,y:cy+(Math.random()-0.5)*16,
        vx:Math.cos(a)*spd,vy:Math.sin(a)*spd-2,life:55+Math.random()*45,maxLife:100,
        emoji:emojis[Math.floor(Math.random()*emojis.length)],size:16+Math.floor(Math.random()*14)});
    }
  }

  bagCanvas.addEventListener('click', e=>{
    const r=bagCanvas.getBoundingClientRect(); tryPunch(e.clientX-r.left,e.clientY-r.top);
  });
  bagCanvas.addEventListener('touchstart', e=>{
    e.preventDefault(); const r=bagCanvas.getBoundingClientRect();
